...
Item {
    ...
    Connections {
        target: oscData
        onNewData: {
            dynamicShader.musicFlat = flat
            dynamicShader.musicLoudness = loudness / 100

            backgroundShader.musicFlat = flat
            backgroundShader.musicLoudness = loudness / 100
        }
    }
    ...
    ShaderEffect {
        ...
        ShaderEffectSource{
            id: noiseShaderSource2
            sourceItem: noiseTexture2
        ...
        fragmentShader: "
            ...
            "void main() {
                 ...
                 vec2 p = uv * 2.0 - 1.0;
                 p *= 15.0;
                 vec2 sfunc = vec2(p.x, p.y + 5.0 * musicLoudness * sin(uv.x * 10.0 + time * 2.0 + cos(time * 7.0) )+ 2.0 * musicLoudness * cos( uv.x * 25.0 - time * 4.0));
